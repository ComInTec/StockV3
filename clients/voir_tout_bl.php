...
<div class="container">
    ...
    function total_versement($id_bl, $link)
    {
        ...
        $req_recette = mysqli_query($link, "      
                ...
                ) AS tab") or die (mysqli_error($link));
...
bl.ref_client=clients.ref_client  ";
    ...
    $bl_print = "where  bl.date = CURDATE( )  ";

    if (isset($_GET["id_bl"]))
        $bl_print = "where bl.id_bl = '" . $_GET["id_bl"] . "' ";


    $req_part3 = " ORDER BY  bl.id_bl DESC";

    $requete = mysqli_query($link, $req_part1 . $bl_print . $req_part3) OR DIE(mysqli_error($link));

    if (mysqli_num_rows($requete)) {

        ?>
        </br></br></br>
        <center><a href="../clients/voir_tout_bl_historique.php"  >Historique</a></center>
